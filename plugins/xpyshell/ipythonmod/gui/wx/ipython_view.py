...
# custom module
from xpyshell import outputmgr
...
class WxNonBlockingIPShell(NonBlockingIPShell):
    ...
    def _yesNoBox(self, prompt):
        ...
        #=====================for xPyshell plugin==============================
        # in case debugging commands are to be processed by the program
        if self.dbginput:
            answer = self.dbginput.pop(0)
        #======================================================================
        #otherwise, process the user input. if user pressed OK, pass the value
        elif dlg.ShowModal() == wx.ID_OK:
            answer = dlg.GetValue()
        # in case of Cancel/close dialog box
        else:
            answer = 'quit'
